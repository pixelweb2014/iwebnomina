<div class="block">
    ...
    <div class="block_head">
        ...
        <ul>
        	<li class="active"><a href="egresos">Listar</a></li>
            <li><a href="egresos/nueva">Nueva Egreso</a></li>
        </ul>
        <ul>
           ...
           <li>Factura: <select id="proveedor" style="width:110px">
            		<option value="">Todas</option>
                    <?php
					foreach($enc_otros_egresos as $kc){
						?><option value="<?php echo $kc['id_enc_otros_egresos'] ?>"><?php echo $kc['num_factura'] ?></option><?php
					}
					 ?>
            </select>
    ...
    <script type="text/javascript">
    	$(document).ready(function(){
			...
			function search_Compra(q, fi, ff, t){
				$.ajax({
						...
						success: function(data){ 
							if(data.length > 0){
								...
								$.each(data, function(i,item){
									html += '<tr class="rows"><td></td><td>'+item.num_factura+'</td><td>'+item.descrip_egreso+'</td><td style="text-align:right">'+item.monto+'</td><td style="text-align:center">'+item.fecha_egreso+'</td><td><a href="egresos/ver/'+item.id+'" class="tip" title="Ver"><img src="public/admin/images/bver.png" /></a>&nbsp; <a href="egresos/editar/'+item.id+'" class="tip" title="Editar"><img src="public/admin/images/bedit.png" /></a> </td></tr>';
			...
			search_Compra($("#descrip_egreso").find("option:selected").val(),$("#fechai").val(),$("#fechaf").val(),0);

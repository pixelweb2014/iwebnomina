...
class Egresos extends Controller 
{
	...
	function index()
	{	
		$data['egresos']  	= $this->egresos->get_all();
		$this->template->display('admin/egresos/list',$data);
		
	}
	
	function listar(){
		$data['value'] = json_encode($this->egresos->get_term());
	...
	{	
		...
		$this->template->display('admin/egresos/nuevo',$data);
	}
	
	
	function editar($id){
		
		$data['data']    = $this->egresos->get_by_id($id);
		$data['detail']  = $this->egresos->get_detail($id);
		
		$this->template->display('admin/egresos/editar',$data);
	...
	function imprimir($id=0)
	{
		...
		$data    = $this->egresos->get_by_id($id);
		$detail  = $this->egresos->get_detail($id);
    ...
    $html2pdf->Output('egresos '.$data[0]['numero'].'.pdf', 'D');
}

...
class Reservaciones extends Controller 
{
	...
	{	
		...
		$codigo          	= $this->reservaciones->get_max_cod();
		$data['cod']     	= $this->codigo($codigo->cod);
        $data['clientes']   = $this->clientes->get_all();


		$this->template->display('admin/reservaciones/list',$data);
		
	}
	
	function listar(){
		$data['value'] = json_encode($this->reservaciones->get_term());
	...
	{	
		...
		$data['cod']     = $this->codigo($codigo->cod);
		$this->template->display('admin/reservaciones/nuevo',$data);
	}
	
	
	function ver($id){
		
		$data['data']    = $this->reservaciones->get_by_id($id);
		$data['detail']  = $this->reservaciones->get_detail($id);
		
		$this->template->display('admin/reservaciones/ver',$data);
	...
	function generar($id){
		...
		$this->template->display('admin/reservaciones/factura',$data);
	...
	function abonos($id)
	{
		...
		$rows = $this->reservaciones->get_abonos($id);
		$row  = $this->reservaciones->get_by_id($id);
		...
		$(document).ready(function(){
			...
			$(".delete").click(function(){
				...
				{
					$.post("reservaciones/delete_abono/",{id:$(this).attr("rel")},function(data){
		...
		function _new(){
			$(".url").attr("href","reservaciones/form_abono/'.$id.'/new");
			$(".url").trigger("click")
		}
		
		function _edit(id){
			$(".url").attr("href","reservaciones/form_abono/'.$id.'/edit/"+id);
			$(".url").trigger("click")
		}
		...
		$html .= "<div id='popup' style='width:450px; height:300px; overflow-y:auto;' class='block'><h2>Reservaci&oacute;n ".$row[0]['numero']." (Deuda = $ ".number_format($row[0]['saldo'] - $row[0]['abonos'],2).") &gt; Abonos</h2>";
		$html .= '<div class="block_content" id="list">
					<a href="reservaciones/abonos/'.$id.'" class="boxdelete"></a>
					<a href="" class="url"></a>
					<div align="right">
	...
	{			
		...
		$res   = $this->reservaciones->get_by_id($idres);
		$row   = $this->reservaciones->get_abono_by_id($id);
		...
		$html  = '<script>
			function back(){
				$.post("reservaciones/abonos/'.$idres.'",{},function(data){
					$.facebox(data);
				})
			}
			$(document).ready(function(){
				$("#form_cobros").validate({
					...
					submitHandler: function(form) { 
						$(form).ajaxSubmit({
								url:"reservaciones/accion_abonos/'.$accion.'",
								type:"post",
								success: function(d){
									if(d == 1){
										 alert("El abono total, supera la deuda de la reservacion");
									}else{
		...
		$html .= "<div id='popup' style='width:500px; height:190px; overflow-y:auto;' class='block'>
				  	<h2>Reservacion ".$res[0]['numero']." (Deuda = $ ".number_format($res[0]['saldo'] - $res[0]['abonos'],2).") &gt; ".$title." Abono</h2>";
	...
	function accion_abonos($opt){
		...
		if($this->reservaciones->validate_abono($_POST['id']) == TRUE)
		{
			echo 1;
			return FALSE;
		}
		if($opt == 'new'){
			$this->reservaciones->add_abono();
		}else{
			$this->reservaciones->update_abono();
		}
	}
	
	function delete_abono(){
		$id = $this->input->post('id');
		$this->reservaciones->delete_abono($id);
	}
	...
	function creditos($id)
	{
		...
		$rows = $this->reservaciones->get_creditos($id);
		$cli  = $this->reservaciones->get_by_id($id);
    ...
    function imprimir($id){
       ...
       // $emp     = $this->empresas->get_by_id(1);
        $data    = $this->reservaciones->get_by_id($id);
        $detail  = $this->reservaciones->get_detail($id);
...
<tr>
    ...
    function crear_factura(){
     ...
     for($i = 0; $i <= count($_POST['ordenes']) - 1; $i++){
        ...
        $_POST['descripcion'][$i] = 'Servicio de montacargas' ;
        $_POST['psi'][$i] = '0';
        $_POST['dscto'][$i] = '0';
        $_POST['quantity'][$i] = 1;
    }
